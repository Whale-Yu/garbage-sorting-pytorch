...
class Train:
    def __init__(self, data_path="./dataset/image", start_epoch=0, epoch=10, lr=0.0001, batch_size=128,
        ...
        # 数据集
        self.train, self.test, self.cls_idx = load_data(data_path, shape=(250, 250), rate=0.8, batch_size=batch_size)
        print(self.cls_idx)
        # 网络 累加训练
        ...
        if os.path.exists(self.module_file):
            ...
            print("加载预训练模型")
            self.net = resnet50(pretrained=True)         # C:\Users\yujunyu/.cache\torch\hub\checkpoints\resnet18-f37072fd.pth
            fc_features = self.net.fc.in_features
        ...
        for e in range(self.start_epoch, self.epoch):
            ...
            for samples, labels in self.train:
                ...
                if self.CUDA:
                    ...
                    labels = labels.cuda()
                # 计算输出
                y = self.net(samples.view(-1, 3, 250, 250))
                # 计算损失
    ...
    def validate(self):
        ...
        for samples, labels in self.test:
            ...
            # 输出
            out = self.net(samples.view(-1, 3, 250, 250))
            # 转换为概率[0, 1)
...
if __name__ == "__main__":
    ...
    lr = 5.5e-5     # 0.000055
    batch_size = 16
    module_file = './model/resnet50_1.pth'
